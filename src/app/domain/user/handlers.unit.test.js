...
describe("handlers", () => {
  describe("createUser", () => {
    ...
    test("When an existing user is not found, creates a user with a hashed password", async () => {
      ...
      require("../authentication-token/generator").createAuthenticationToken.mockResolvedValue(
        { userId: 1, token: "authentication-token" }
      );
      ...
      await expect(UserHandlers.createUser(req)).resolves.toEqual({
        user: createdUser,
        token: "authentication-token",
      });
      expect(require("./store").UserStore.insertUser).toHaveBeenCalledWith(
        createdUser
      );
  ...
  describe("authenticateUser", () => {
    test("Throws an error if the email provided does not match a user in our database", () => {
      ...
      expect(UserHandlers.authenticateUser(req)).rejects.toThrow(
        new HTTPError(400, "Invalid email address or password")
      );
    });

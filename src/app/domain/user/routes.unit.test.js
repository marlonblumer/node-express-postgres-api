  ...
  describe("UserRouter", () => {
    ...
    describe("createUser", () => {
      test("throws an error if a required property is not present in the request", () => {
        ...
        expect(userRouter.createUser(req, res)).rejects.toThrowError(
      ...
      test("When validation passes, call UserHandlers.createUser", async () => {
        ...
        require("./handlers").UserHandlers.createUser.mockResolvedValue({
          token: "",
          user: {},
        });
        ...
        expect(
          require("./handlers").UserHandlers.createUser
    ...
    describe("authenticateUser", () => {
      test("throws an error if email address is not present in the request", () => {
        ...
        expect(userRouter.authenticateUser(req, res)).rejects.toThrow(
      ...
      test("throws an error if email address is not a string in the request", () => {
        ...
        expect(userRouter.authenticateUser(req, res)).rejects.toThrow(
          "Email must be a string. Password is required"
        );
        expect(res.json).not.toHaveBeenCalled();
      });
      ...
      test("calls res.json with a user / token once validation has passed", async () => {
        ...
        require("./handlers").UserHandlers.authenticateUser.mockResolvedValue({
          token: "",
          user: {},
        });

        const userAuthenticateResponse = await userRouter.authenticateUser(
          req,
          res
        );
        ...
        expect(jsonSpy).toHaveBeenCalled();
        expect(
          require("./handlers").UserHandlers.authenticateUser
        ).toHaveBeenCalledWith(req);
      });
    });
  });

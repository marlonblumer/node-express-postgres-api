...
describe("AuthenticationTokenStore", () => {
    ...
    test("Calls query with the correct parameters in the expected order", async () => {
      ...
      const dbSendMock =
        require("../db").postgres.client.query.mockResolvedValue({
          rows: [authenticationTokenParams],
        });

      const res = await AuthenticationTokenStore.insertToken(
        authenticationTokenParams
      );

      const functionInputs = dbSendMock.mock.calls[0];

      expect(functionInputs[0]).toEqual(
        'INSERT INTO "authentication-tokens"("userId", token) VALUES($1, $2) ON CONFLICT ON CONSTRAINT "authentication-tokens_userId_key" DO UPDATE SET token = $2 RETURNING *'
      );
  ...
  describe("getToken", () => {
    test("Calls query with the correct parameters in the expected order", async () => {
      ...
      const dbSendMock =
        require("../db").postgres.client.query.mockResolvedValue({
          rows: [authenticationToken],
        });

      const res = await AuthenticationTokenStore.getToken(
      ...
      expect(functionInputs[0]).toEqual(
        'SELECT * FROM "authentication-tokens" WHERE token = $1'
      );
  ...
  describe("deleteToken", () => {
    test("Calls query with the correct parameters in the expected order", async () => {
      ...
      const dbSendMock =
        require("../db").postgres.client.query.mockResolvedValue({ rows: [] });
      ...
      expect(functionInputs[0]).toEqual(
        'DELETE FROM "authentication-tokens" WHERE "userId" = $1'
      );
